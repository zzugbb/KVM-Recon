...
本记录陈述 0.3.0 发布前实际执行的回归验证；历史包和现场 HAR 均只读，未修改仓库外资料，也未把 1.x 包冒充 2.0 包。它不代表对所有设备或固件的兼容性保证。
...
| macOS 产物 | x64/arm64 DMG 与 ZIP 均生成，DMG 校验、ZIP 解压测试、应用签名校验通过；arm64 已打包应用在隔离用户目录完成启动烟测 | ad-hoc 签名且未公证 |
...
验证依据为 [开发规范 §20](v0.3-development-spec.md)。本项目不推断适配任何具体协议族，也不改 InManage 或历史资料。
