...
**0.3.0 的采集、证据整理、导出和单作业界面已落地；当前源码版本为 0.3.0。**
...
- 阶段 5（界面收口）：`stage.ts` 八阶段纯函数派生（idle/launching/capturing-login/capturing-viewer/finalizing/complete/incomplete/exported + 四步阶段条三态；已收尾但未派生 COMPLETE 一律按 incomplete，不主张未验证的完整）；界面重排为规范 §5 单屏单作业工作台（当前状态面板承载阶段、提示与主操作，醒目作业标题，稳定宽度计数器、最近事实 feed、高级诊断默认折叠、lucide-react 图标）；`capture:status` 载荷扩展（计数器 / 包工件字节记账 / 收尾后预导出完整度 / 最近事实 / 磁盘余量与日志尾部）；导出收口（INCOMPLETE 按钮明确「导出未完整包」、「打开所在文件夹」路径由主进程决定、「采集下一台」丢弃清理）；删除 0.2.x 死 CSS 选择器。
...
- 阶段 6 发布前回归：现场/家庭旧包只读清点、第二批现场 HAR 离线回放、Mock 端到端、本机 macOS 双架构打包及一台家庭 BMC 的 2.0 实采与旧包对比已验证；该设备 Viewer 的 `No Signal` 正是现场正常 KVM 画面。详见 [阶段 6 验证记录](phase-6-validation.md)。

0.3.0 发布前本地回归已记录；其他设备和固件的覆盖情况以实际采集证据为准，不据此宣称通用适配成功。
