...
10. 导出前可点“关闭采集窗口”：窗口关掉后作业数据仍保留，可继续导出。也可直接点“停止采集并导出”。导出成功后可“再次导出”。
11. 导出前会显示离场结论和脱敏摘要，确认后再选择保存位置。导出成功后采集窗口会关闭。
12. 打开 `report.html` 或 `report.md`，确认离场结论。出机房后把 zip 交给工程师或 AI，并阅读 `artifacts/handover.md`。本工具不写 Adapter。
...
- 同一时刻只保留一份可导出作业。关闭采集窗口后仍可导出；再点“新建采集作业”若尚未导出会先确认。
