...
11. 导出前可点“关闭采集窗口”：窗口关掉后作业数据仍保留，可继续导出。也可直接点导出按钮（资料已齐时为「导出 Capture Pack」，未齐时为「停止采集并导出」）。导出成功后可“再次导出”。
...
- `YES`：关键资料完整，可以离场后分析。
- `PARTIAL`：可以分析，但存在警告项，建议按报告补采。常见原因包括：唯一的登录/KVM Token/启动接口还在途或没有正文、Viewer/Worker 源码没采到、OOPIF/Worker 附加失败。BMC 页面上已经成功采过的重复轮询，导出瞬间仍在途时**不会**单独把整包打成 PARTIAL；`http/capture-status.json` 里仍能看到这些 in-flight ID。
