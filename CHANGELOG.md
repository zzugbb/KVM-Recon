...
- Capture Pack 2.0：保存 Chromium 可观察的 HTTP/HAR、脚本与 Worker/WASM、浏览器状态、画面、实时通道及加密调用；包内提供证据索引、值传播与 Replay 资料，缺失资料显式记账。
- 单作业磁盘工作区、内容寻址正文存储和流式 ZIP64 导出，取消旧版正文、文件数量与 WS 帧采样上限；导出仍由用户手动触发。

### Changed

- 完整度只评价工作流与证据门禁；未知协议也可得到 COMPLETE，缺失或不可读取的资料必须明确记为 INCOMPLETE。
- 工作台改为单作业流程，支持崩溃恢复、原始资料保留和未完整包导出；HAR 大正文按块写出，不再为生成副本整份载入内存。
- 工作台以“当前状态”面板为主：阶段标题、下一步提示和主操作集中在面板内；空闲时面板承载新建作业表单，采集开始后顶部显示醒目的 BMC 地址与设备说明。操作错误、不完整原因（中文说明 + 稳定代码）和导出路径都在面板内展示。
- 计数器改为大号数字指标条，最近事实最新在上；高级诊断展开后与最近事实分摊高度，不出现嵌套滚动。大屏下内容宽度与字号随窗口放大，顶栏显示应用图标并突出未脱敏标识。
- 恢复作业存在现场证据时，“丢弃恢复作业”直接置灰并说明原因；恢复作业导出、丢弃或保留后的结果提示可以关闭。
- TLS/Redfish 仅作为通用补充事实；包内资料未脱敏，应按敏感数据保管。

### Removed

- 移除采集侧厂商/协议分类、离线 Analyzer，以及当前源码中不再使用的 1.x 兼容代码、Schema、样例和文档。已发布版本仍可从对应 Git 标签获取。
