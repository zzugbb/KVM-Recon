...
- 0.3.0 阶段 2 收口：生产 Controller（每作业独立 partition、自签证书放行、popup 血缘、`about:blank` 预提交）与单作业 Electron 壳（IPC start/status/stop/export/discard、启动崩溃恢复 `recoverCrashedJob` + 手动导出 `exportRecoveredJob`、保守证据摘要）替换 0.2.x 链路；删除 0.2.x 运行链路与五项大小上限（1 MiB / 2 MiB / 24 文件 / 8 MiB / 64 帧），导出不脱敏，恒为 `INCOMPLETE + TARGET_OPENED`（workflowStatus 由阶段 3 完整度引擎派生）。
...
- 0.3.0 阶段 3（证据图与完整度）：`workflowStatusEngine` 从只读会话事实快照派生三态——`LOGIN_REACHED` 要求 POST + 正文 + 2xx/3xx + 响应 Set-Cookie 且**之后**有请求逐字节携带该 cookie，`KVM_REACHED` 要求 Viewer 活动严格按“用户动作 → popup/显著导航 → 新渲染/执行表面 → 持续双向通道”出现，否则 `TARGET_OPENED`；识别失败退回诚实下限并显式记账，绝不阻断采集与收尾；观察脚本钩子失败仅当对应观察面真实在场才映射 `channelGaps` 缺口。
...
- 0.3.0 阶段 3 第 12 轮审核修复（5 阻断 + 2 附加）：Viewer 活动识别按 target 血缘收紧（`deriveViewerActivity` / `detectViewerActivity` 两套实现同步）——动作 target 与由它经 `openerTargetId` 链打开的 popup 后代构成血缘集合，导航/通道须归属该集合且通道创建不早于打开证据，`action.targetId` 缺失为零信号（保守方向），无关窗口的导航/通道不再误判 KVM_REACHED 过早自动收尾；`workflowFactsSignature` 投影同步覆盖新增读取面，看门狗指纹计入导航数（新导航重置 15s 稳定窗口）。Worker 跨 session 唯一 URL 别名回归（0.2.9 语义按新架构恢复）：父会话请求在 Worker session 收到完成事件时经 `sessionId::requestId` 别名命中父 hop、正文按事件所在 session 补读，Worker target URL 多候选不建别名（宁可漏不可错），Worker session detach 时未完成别名请求强制收尾并显式记账。收尾 drain 期照常处理在途请求完成事件（`responseReceived` / `loadingFinished` / `loadingFailed` / ExtraInfo）；从未收到完成事件的在途请求（status=null）在 flush 收尾时逐条记入 `missingBodies` 缺口（`INCOMPLETE_BODY_MISSING`），不再伪装成明确无正文（loadingFailed / 显式 204、205、304 / redirect 仍是明确无正文语义，不作证缺失）。浏览器状态快照逐步骤判定（cookies / storage / IndexedDB / CacheStorage / frameTree）：任一步骤失败或返回形状异常 → `browserStateGaps` 缺口 → `INCOMPLETE_BROWSER_STATE`，`browserStateWritten` 不再假成功。证据图生成 / storage 读取失败 → `evidenceGraphFailures` 缺口 → `INCOMPLETE_EVIDENCE_REFERENCE`，`evidenceReferencesClosed` 只在证据图完整闭环时置位。响应到达时间改用 CDP `receiveMs` 累计偏移（`startedAt + max(receiveMs, sendMs + waitMs)`，拒绝边方向保守）；HAR `time` 不再对 `receiveMs` 双重计数（receive 段记 -1，正文时长不可知）。崩溃恢复拆分为 `recoverCrashedJob` + `exportRecoveredJob`：恢复止步于 finalize，不再启动即写下载目录 ZIP 并自动打开目录；恢复卡提示待导出状态，用户手动选择目录导出（新 IPC `capture:exportRecovered`），ZIP 自检成功才 `markExported`；取消/失败保留待导出状态，下次启动幂等再恢复提示。
- 0.3.0 阶段 3 第 12 轮后第三轮自审修复（1×P2 + 9×P3）：收尾截图与 DOM 快照纳入浏览器状态步骤明细（七步全进 `browserStateGaps` 缺口，截图/DOM 快照失败不再绕过 `INCOMPLETE_BROWSER_STATE` 门禁）。二次恢复不再把恢复覆写的保守摘要（`recovered=true` 痕迹）标成「真实摘要」。drain 期 Worker 别名回退只对父会话已跟踪的 requestId 建立（requestWillBeSent 已丢弃的新请求不得凭 URL 唯一性错关联在途父请求）。`patchHop` 对已 commit 的行拒绝改写（journal 只追加，晚到的 responseReceived 状态/头与晚到的响应正文显式记 `missingBodies` 缺口，不再无痕写内存行）。Worker session detach 强制收尾按「未完成」处理：缺正文 hop 逐条记 `missingBodies` 缺口 + detach 原因显式记账（204/205/304/redirect 等明确无正文语义除外），不再伪装成观察到的 loadingFailed。`capture:exportRecovered` 增加导出进行中重入防护（并发 invoke 不再可能写出两份 ZIP / 双重 markExported）。恢复卡待导出提示补渲染设备说明，「已导出」提示照实显示本次导出的完整度（不再硬编码 INCOMPLETE）。
- 0.3.0 阶段 3 第四轮审查修复（1×P1 + 1×P2 + 5×P3）：`capture:exportRecovered` 不再置 `lastExport`（恢复作业不属于任何当前作业，置位会让下一个新作业从首轮 status 起报 `exported`，UI 无法停止/导出/丢弃；恢复导出事实由 recoveryNotice 独自承载）。`requestWillBeSent` 的 redirectResponse 分支接线 `patchHop` 返回值：已 commit 前跳的晚到 302 状态/头/redirectToId 显式记账，不再无痕丢弃。`storeRequestBody` / inline postData 两处 `patchHop` 返回值同样接线（跨附件交错收尾窗口的晚到请求正文显式记 `missingBodies` 缺口）。`loadingFinished` 正文读取失败不再记 `missingBodies` 假缺口：读失败事实按 droppedEvent 记账，缺正文是否构成缺口由 commit 单点判定（204/205/304/redirect 明确无正文语义不作证缺失，也不再双重记账）。Worker detach 强制收尾按 hop 隔离：单个 commit 写盘失败不中断其余 hop 收尾、不跳过别名/会话清理。drain 期别名信任门收紧为「链非空」：先到的未知完成事件建出的空链不构成已跟踪证据，不得解锁 URL 回退别名。恢复拒绝（refused）路径释放工作区句柄与 `.owner` 租约（现场资料保留在磁盘，句柄不滞留到进程退出，下次启动照常幂等再接管）。
- 0.3.0 阶段 3 第五轮外部审计修复（6 阻断 + 2 完整性缺口 + 1 弃用警告）：popup Viewer 收尾补采浏览器状态快照（验证器按 viewer-initial / stop 两阶段标签断言，不再只验证最终快照）。自动收尾等待非持续 HTTP 在途请求完成（drain 白名单补 `Network.getResponseBody` 及 ExtraInfo 关联事件，viewer 停止后到达的完成事件照常落盘）。LOGIN_REACHED / value-flow 响应到达时间改用 CDP `receiveMs` 累计偏移（`startedAt + max(receiveMs, sendMs + waitMs)`；`receiveMs` 缺失退回第 11 轮近似，两级下限取更晚者——拒绝边方向保守）。CacheStorage 正文落盘失败翻转 `cacheStorageOk=false`（步骤失败不再伪装成功，反例先行验证）。`Controller.stop()` 并发幂等（`stopPromise` 复用，并发调用共享同一次收尾序列——含认证 probe 的网络副作用不重复执行）。Worker attach 时 `targetInfo.url` 为空的迟到 URL 补齐：`Target.targetInfoChanged` 到达时补写 `workerSessions`；若该 Worker session 在 URL 为空期间已有完成事件按未命中显式丢弃（不可重放），在 URL 补齐时刻经 Worker session 补读入口脚本正文（父请求原始 requestId 在 Worker session 上调 `Network.getResponseBody`，0.2.10 `salvageWorkerMainScript` 的 0.3 重建）；URL 多候选父请求不补读不建别名（宁可漏不可错）。空源码完整度记账按失败位置三向区分（`Debugger.getScriptSource` 成功但源码为空 → 任何 kind 记缺口；失败（Chromium 导航/文档销毁竞态已回收脚本，不可重读）→ 采集层 `droppedEvent` 显式记账，§8.3 一等证据的 Worker/WASM 仍记缺口，inline/eval/network script 不虚构完整度事实；`scriptParsed.length=0` → 内容本就为空，0 字节 bodyRef 即完整内容无缺口）。`console-message` 迁移到 Electron 44 事件对象签名（`{ message, level, lineNumber, sourceId }`，旧五参数签名已弃用）。全部 e2e 双跑确认：完整 mock 会话仍 `COMPLETE + KVM_REACHED`。已知边界（交审计裁决）：Chromium 侧 `responseReceivedExtraInfo` 偶发乱序/迟到（实测 ~1/39）时 collector 按 §3 显式 `droppedEvent` 记账，修复需违反 journal 只追加红线，维持诚实丢弃。
- 0.3.0 阶段 3 最终整体复核：popup 独立留存 Storage 上下文；iframe/OOPIF/Worker 子 target 通过 `parentTargetId` 进入 Viewer 血缘，子 target 的 Viewer 初始截图路由到所属根窗口；手动/自动 stop 等待在途初始截图落定。`getScriptSource` 失败不再写 0 字节假正文，仅可用相同 CDP hash 的已采正文精确补全，否则任意脚本 kind 都记完整度缺口；独立一致性验证器拒绝脚本索引无 BodyRef 却声明 COMPLETE 的包。本条脚本策略取代上一条 G8 中“非 Worker 只记 droppedEvent”的权衡。
- 0.3.0 阶段 3 收口复核：Viewer 四组事实新增严格顺序反例，Dashboard 已有 Canvas 与后台 WS 不得跨导航拼成假阳性；采集器自身的 Observer、Storage、DOM、环境等 `Runtime.evaluate` 探针统一标记 `kvm-recon-internal://` 并排除出目标脚本索引，避免导航回收内部探针时误报 `INCOMPLETE_WORKER_SOURCE`，真正页面/Worker/WASM 源码门禁仍保持严格。Mock KVM 完整采集连续回归为 `COMPLETE + KVM_REACHED + UNKNOWN`。
- 0.3.0 阶段 4（AI/Replay 生成器，装配时派生）：`readPackFacts` 只凭包内工件重建 `WorkflowFacts`（主框架导航从 `raw/cdp/events.jsonl` 重放 `Page.frameNavigated` 无 parentId 帧、钩子失败从 observer-hook-failed payload，与在线采集同一规则；缺文件/坏行记入派生缺口，派生退化绝不阻断导出）；`dossierEngine` 七角色候选链（login-interaction → session-established → kvm-click → launch-request → viewer-opened → script-worker-wasm → realtime-channel，每步 evidenceIds/evidencePaths/occurredAt，复用 loginChainOf / detectViewerActivity 同一判定）与 `ai/index.json` 全部候选列表；`replayEngine` 派生 `replay/manifest.json` + `replay/http.jsonl` + `replay/channels.json`——`requiresDynamicValueIds` 只引用值传播图 replaySubstitution 边的来源值节点，不可回放时 `replayable=false` + `notReplayableReasons` 逐条说明缺什么资料（replay-manifest Schema 扩展，`clockPolicy=realtime`）。
