...
## 检查项
- BMC 基础连接：pass / blocking；证据：https://10.0.0.10:443, tls:TLSv1.2, http:200,200；补采动作：无
- BMC 协议族指纹：pass / warning；证据：ami-megarac:0.9, /api/randomtag, http:/api/session, http:/api/kvm/token；补采动作：无
...
- 网络响应采集完整性：pass / warning；证据：timedOut=false, pendingTaskCount=0, materialPendingCount=0, inFlightRequestCount=0, materialInFlightCount=0；补采动作：无
